...
def calculate_channel_loss(generated_image, target_image, channel):  
    gen_hsv = cv2.cvtColor(generated_image.cpu().numpy(), cv2.COLOR_RGB2HSV)  
    target_hsv = cv2.cvtColor(target_image.cpu().numpy(), cv2.COLOR_RGB2HSV)  

    if channel == 'H':  
        gen_h, _, _ = cv2.split(gen_hsv)  
        target_h, _, _ = cv2.split(target_hsv)  
        return F.mse_loss(torch.tensor(gen_h), torch.tensor(target_h))  

    elif channel == 'S':  
        _, gen_s, _ = cv2.split(gen_hsv)  
        _, target_s, _ = cv2.split(target_hsv)  
        gen_gram = gram_matrix(torch.tensor(gen_s))  
        target_gram = gram_matrix(torch.tensor(target_s))  
        return F.mse_loss(gen_gram, target_gram)  

    elif channel == 'V':  
        _, _, gen_v = cv2.split(gen_hsv)  
        _, _, target_v = cv2.split(target_hsv)  
        return F.mse_loss(torch.tensor(gen_v), torch.tensor(target_v))  

def add_channel_level_perturbation(generated_image, clean_image, perturbation_strength, brightness_weight, fractal_weight, style_weight):  
    h_loss = calculate_channel_loss(generated_image, clean_image, 'H')  
    s_loss = calculate_channel_loss(generated_image, clean_image, 'S')  
    v_loss = calculate_channel_loss(generated_image, clean_image, 'V')  

    total_loss = (brightness_weight * h_loss + fractal_weight * f_loss + style_weight * s_loss)  
...
def calculate_fractal_dimension(image, box_sizes=None):  
    ...
    try:  
        ...
        m, c = scipy.linalg.lstsq(A, log_counts)[0]   
        return m   
...
def perturbation_until_target_confidence(  
        ...
        if step >= perturb_start_step:  
            ...
            for i in range(len(images)):  
                if len(images[i].shape) == 2:   
                    images[i] = images[i].unsqueeze(0).repeat(3, 1, 1)   
            ...
            for gen_box, ref_box in zip(original_boxes, ref_boxes):  
                l_inf_loss, cosine_loss, style_loss, total_loss = calculate_loss(generated_image, ref_image, gen_box, ref_box, lambda_inf, lambda_cosine, layers=None)  
    ...
    if detected_boxes is not None and len(detected_boxes) > 0:  
        ...
        plot_boxes_on_image(generated_image_pil, formatted_boxes)  

    clean_image_pil = transforms.ToPILImage()(original_clean_image.squeeze(0).cpu())  
    ref_image_tensor = ref_image.unsqueeze(0)  
